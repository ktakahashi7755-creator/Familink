...
- 既存の PERSIST 配列（現在 88 キー・2026-07-08 時点）を壊さない。**新規保存キーを足す場合は PERSIST 配列にも必ず追加**する
...
- **SW はキャッシュ優先（cache-first）方式**：開いた瞬間にキャッシュから即表示＝遅い回線でも待たせず
  ...
  （sw.js のバイトが変わることで更新が検知され、skipWaiting → controllerchange で **1 回だけ自動リロード**して
  新版へ切り替わる。詳細は `docs/spec-v3/12-operations-release.md` §4）。
- **同期コマンド**（毎回このパターンで実施）:
  ```sh

...
| `backup/029-wave136-calendar-readability` | **Wave 127-136 — 予定/タスクの繰り返し・祝日表示・週ビュー祝日・カレンダー見やすさ改善・アルバム追加簡素化・デモ初回限定 最新** ⭐ |

すべて remote（origin）に push 済み。`git checkout backup/029-wave136-calendar-readability`
で Wave 136 時点（コミット `7a8c9a2`）に即復元できる。

### 復元例（Wave 136 の状態に戻す）

```bash
git checkout claude/familylink-unicorn-product-TzM1F
git reset --hard origin/backup/029-wave136-calendar-readability
```

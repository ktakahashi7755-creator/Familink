...
## 2026-05-31 (2)  env: 不明  branch: claude/latest-version-device-check-652i3

### 作業名
全ページ総点検：絵文字除去・準備中表現修正・小フォント修正
...
### 変更内容
- **絵文字除去（5箇所）**：📩（OTPメール説明）/ 📱（ローカル開始ボタン）/ 🔍（接続診断リンク）/ 🔥（ホーム連続日数バッジ → 「N日連続」テキストに変更）/ 📱（Supabaseバナー内テキスト）
- **⚠ 除去**：体調記録画面の緊急時案内から ⚠ 記号を削除（テキストは保持）
- **Hoku「準備中」表現修正**：calendar_import_help「自動同期は準備中」→「今は手動取り込みに対応してるよ」、Googleカレンダー連携「Google連携は準備中」→「Googleカレンダーの予定ファイルで取り込めるよ」
- **小フォント修正（3箇所）**：.cal-cell-more（9px→11px）、ルーティンチップ（9px→11px）、アバター表示（9px→11px）
- docs/index.html 同期・キャッシュバスター v20260531k → v20260531l に更新

### テスト結果
- Playwright QA全84テスト：84 PASS / 0 FAIL / 0 WARN

### 未確認事項
- iPhone 実機で連続日数バッジの表示確認（「N日連続」テキスト）

### iPhone確認ポイント
- ホーム画面の連続日数バッジが「3日連続」のようなテキストで表示されること
- 体調記録画面の緊急時案内バーが ⚠ なしで違和感ないか

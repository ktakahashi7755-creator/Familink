...
## 2026-06-23 10:30  env: Remote（claude.ai/code 経由）  branch: claude/peaceful-hawking-ab2tbk

### 作業名
ランディングページフッターから法的文書リンク（利用規約・プライバシーポリシー）を削除

### 変更ファイル
- app-source/familink.html
- docs/index.html（v20260613g へバージョンアップ）

### 変更内容
- ob2-footer ブロック（利用規約・プライバシーポリシーリンク）を削除
- ランディングページのフッターが不要になったため、スッキリさせる
- app-source で修正後、docs/index.html へ同期（バージョン v20260613f → v20260613g）

### テスト結果
- QA 自動テスト: **84/84 PASS** ✅
- 回帰確認: すべてのスクリーン・機能・バリデーション・レイアウト正常

### 未確認事項
- なし

### iPhone確認ポイント
- なし（マークアップ削除のみで機能変更なし）

### 次にやること
- 実機リンク招待テストの結果待機（前回の申し送りから継続）

### コミット
- ハッシュ: `51b3782`
- メッセージ: `fix: ランディングページフッターから法的文書リンクを削除`

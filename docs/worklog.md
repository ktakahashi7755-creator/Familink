...
## 2026-06-15 07:30  env: iPhone経由  branch: claude/latest-version-link-nvsjm9

### 作業名
ログイン導線を「通常ログイン（メール＋パスワード）」のみに統一＝OTP/マジックリンクを表に出さない

### 変更ファイル
- app-source/familink.html／docs/index.html(v20260615a)／worklog

### 変更内容
- **既定モードを otp → signin に変更**（openSupaAuthModal の既定 / ホームのクラウド誘導バナー onclick）。
  認証モーダルを開くと最初から「メールアドレスでログイン」＝メール＋パスワードのみが出る
- **OTP/マジックリンクへの可視導線を撤去**: フォーム内の「← メールでコードを受け取る方法に戻る」ボタンを削除
- **エラーバナーの「メールでコードに切替」誘導（5箇所）を撤去**し、
  「パスワードを再設定する」または通常ログイン案内に置換（既登録/Invalid login/Email not confirmed/needsConfirm 等）
- ヘルプ文（よくある原因と対処）の OTP 案内をパスワード再設定案内へ修正
- OTP/otp-code のUI・関数自体は後方互換のためコード上は温存（表からは到達不可）。認証バックエンドは不変
- 既定ログイン表示は「ログインする」「メールアドレスでログイン」のみ

### テスト結果
- node qa_full_test.js: **84/84 PASS**（コンソールエラー0）
- 認証系 tools: supaerr 4/4・authguard 7/7・cloudfirst_login 9/9・invite_link 13/13・autosync 10/10・e2e_flow 14/14 全緑
- Playwright 実挙動確認: 既定起動でOTP欄非表示・OTP戻りボタン消滅・pageerror 0／
  signinモードで「メールアドレスでログイン」「ログインする」「パスワードをお忘れですか？」のみ表示

### 未確認事項
- ユーザー報告「ホク（北）が文字になって反映されていない」: 当方で再現できず（QAのコンソールエラー0・レイアウト緑）。
  キャッシュ起因の可能性が高く、今回 docs を v20260615a にバンプ済み。実機で再読み込み後も文字化が残るなら該当画面のスクショが必要
- 「認証方式の表示変更」は §14.3 の要確認領域だが、ユーザー明示指示のため §9 に従い実施（バックエンド認証は不変・UI 既定の切替のみ）

### iPhone確認ポイント
- ログイン（クラウド連携）を開くと最初から「メールアドレス＋パスワード」だけが出るか（「ログインコードを送る」が出ないこと）
- ボタン文言が「ログインする」になっているか／「パスワードをお忘れですか？」「新規登録」が機能するか
- 旧バージョン表示が残る場合は一度再読み込み（v20260615a）
- 「ホクが文字化」が再現するか（再現するなら画面名とスクショを）

### 次にやること
- 実機でログイン画面の最終確認。ホク文字化が再現した場合は対象画面を特定して修正
...
## 2026-06-15 10:50  env: iPhone経由  branch: claude/latest-version-link-nvsjm9

### 作業名
総点検＋3点改善：同期失敗バナーを「セッション中1回のみ」/ ウェルカム画像の堅牢化 / 全スイート緑確認

### 変更ファイル
- app-source/familink.html／docs/index.html(v20260615b)／worklog

### 変更内容
- **同期失敗バナーを“1回だけ”に（最重要・煩わしさ解消）**: `_updateNetBanner` に
  セッション単位ラッチ `_netErrNotified` を追加。`error` 状態の自動表示は一度きりにし、
  以後の断続失敗は赤い同期ドットのみで静かに提示。20秒ポーリング/フォーカス復帰/オンライン
  復帰での自動再試行は従来どおり継続（機能は落とさず通知だけ静音化）
- **ウェルカム画面の画像を堅牢化（「四角い枠＋Hoku文字」対策）**:
  - `ob-hoku-star` は `src` 未設定＋`alt="Hoku"` のため、JS適用前/別経路表示で
    “壊れ画像の四角＋altテキスト”が出る潜在バグ → `alt=""`（装飾）＋`onerror=display:none` に
  - 家族イラスト(hero) にも `onerror=display:none` を付与
  - ※現行ビルドでは両画像とも正常ロード（naturalWidth 240/640・崩れ無し）を実機相当の
    Playwright スクショで確認済み。実機で残るのは旧キャッシュが原因の可能性が高く v…b にバンプ
- SW はネットワーク優先（キャッシュ不使用）。版バンプ＋再読み込みで最新反映

### テスト結果
- node qa_full_test.js: **84/84 PASS**（コンソールエラー0）
- tools 全スイート: **274/274 PASS**（fam_ocr/ocr_unit/perf_lazy/tour 系含む・pageerror 0）
- バナー1回制御の実挙動: error→表示 / synced→消える / 再error→自動表示しない（ラッチ動作確認）
- ウェルカム実描画: 星・家族イラスト正常、文字化/四角枠なし（スクショ確認）
- Vitest(unit): 未実施（理由: サンドボックスに vitest 未インストール・npm依存ゼロ方針＋ネット制限）

### 未確認事項
- 実機での「同期そのものの失敗」原因特定（当方はサンドボックスから本番Supabaseへ到達不可）。
  既知の最有力要因＝Supabase Auth の Confirm email ON（第2ユーザーがログイン不可で同期しない）。OFF推奨
- ログイン/家族共有は招待リンク方式・パスワードログインで実装済み・テスト緑だが、実機2台での通し確認は要

### iPhone確認ポイント
- ログイン後、同期に失敗しても「同期に失敗しました」バナーが**1回だけ**で以降出ないこと（赤ドットのみ）
- ウェルカム画面に四角い枠/「Hoku」文字/砂目が出ないこと（旧表示が残る場合は再読み込み→v20260615b）
- 2台でリンク招待→参加→予定が双方向に同期するか

### 次にやること
- 実機2台で同期の通し検証。失敗するなら #qa-debug のセルフテストで止まるレイヤーを特定
- （バックエンド）Supabase Auth の Confirm email 設定確認

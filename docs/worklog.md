...
## 2026-06-16 02:30  env: iPhone経由  branch: main

### 作業名
全バックアップ作成＋再構築用「完全仕様書セット」(docs/spec-v3・15文書)を作成

### 変更ファイル
- docs/spec-v3/（新規15文書：README＋00〜13）／worklog
- バックアップ枝 backup/2026-06-16-v20260615j-full-suite（push済み・復元可）

### 変更内容
- 全機能/全デザイン/世界観を読み取り、ゼロから再構築できる超プロフェッショナル仕様書を作成:
  00概要/事業戦略・01要件定義書・02基本設計/アーキ・03データモデル・04画面設計・
  05同期/リアルタイム・06セキュリティ/RLS・07Hoku AI・08課金/Stripe・09通知/WebPush・
  10デザインシステム・11テスト計画・12ゼロから再構築手順・13用語集/ロードマップ
- 実コードから正確な事実を抽出（22画面・PERSIST88キー・SYNC/共有キー・課金上限・トークン・Edge6本）
- アプリ本体(app-source/docs/index.html/sw.js)は不変

### テスト結果
- アプリ本体変更なし＝挙動不変（QA対象の変化なし）。秘密情報の混入0（スキャン済み）
- バックアップ枝のリモート存在を確認（復元コマンドを12章に明記）
...
### 次にやること
- （ユーザー）Web Push / Stripe の本番設定 → 実機2台の同期end-to-end

...
# テスト整備・最終QA（Test & Release）

- [x] **T1 Vitest ユニットテスト** — 実コード抽出ハーネスで日付/課金境界/権限・招待/バリデーション/繰り返しを検証(23件)。
- [x] **T2 主要フロー結合テスト** — 家族作成→招待→予定→タスク→買い物→アルバムの通し(14件)。
- [x] **T3 全テストPASS** — Vitest23 + Playwright各スイート + QA84、全て緑。
- [x] **T4 RELEASE-CHECKLIST.md 作成** — 機能/セキュリティ/実機/既知課題。

...
- [ ] **C2 招待コードの露出窓を閉じる** — family_id ベアラ問題への対策（→ H3 と一体）。
- [x] **C3 シークレットのクライアント露出ゼロ確認** — service_role / API キー非搭載を機械検証。

## High
- [ ] **H1 Storage バケットポリシー** — 現状未使用（N/A）。将来移行用の RLS ひな型を用意。
- [ ] **H2 ファイルアップロード検証** — 非画像/非動画/巨大ファイルを各入口で明示拒否。
- [ ] **H3 招待コードの有効期限・使い捨て化** — fl_family_invites テーブル＋redeem RPC（SQL）。
- [ ] **H4 セッション/未認証リダイレクト検証** — 未ログイン時に保護画面へ確実に戻ることを検証。

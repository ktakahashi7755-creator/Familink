...
最終更新：2026-06-06（Wave 215h 反映） / 状態：**Phase 4-2 / 4-3 実装済み・Phase 4-1 の実行用 SQL を確定（`docs/supabase-setup-sql.sql`）→ 次はオーナーが SQL を実行 → Phase 4-4 / 4-5 着手**

> **▶ オーナーへ（最重要・ここがブロッカー）**：複数端末の家族共有を「実際に動かす」には、まず
> **`docs/supabase-setup-sql.sql` を Supabase の SQL Editor に全文貼り付けて 1 回実行**してください。
> これでテーブル・RLS・招待コード・Realtime が一括構成されます。これは DB 側の作業のため
> Claude では実行・検証できません（§10.2 オーナー確認必須・本物の家族データに関わる重大変更）。
> 実行後にアプリ側の同期レイヤー（Phase 4-4/4-5）を実装し、2 アカウントで受け入れ条件を検証します。
...
> **実行用の最終版 SQL は `docs/supabase-setup-sql.sql` に確定済み**（テーブル + RLS + 招待コード
> RPC + Realtime を冪等にまとめた、貼り付けて 1 回実行するだけの完成スクリプト）。
> 以下はその設計の解説。実際に流すのは `.sql` ファイルの方を使ってください。
...
|---|---|---|
| 4-0 | 本設計書（確認）| ☑ 本書 |
| 4-1 | オーナーが Supabase プロジェクト作成 + SQL 実行 | ◐ プロジェクト作成 + 公開キー受領済 / **実行用 SQL を確定（`docs/supabase-setup-sql.sql`）→ オーナーの実行待ち** |
| 4-2 | ログイン / サインアップ / 招待コード画面（UI）| ☑ Wave 202 で実装（招待は受付スタブ） |
| 4-3 | supabase-js 連携・認証フロー | ☑ Wave 202 で実装（onAuthStateChange / セッション永続化） |
| 4-4 | family_data 同期レイヤー（読み書き + 移行）| ☐ **Phase 4-1 の SQL 実行後に着手（DB が無いと実装も検証もできないため意図的に未着手）** |
| 4-5 | Realtime 購読（複数端末リアルタイム反映）| ☐ 4-4 と同時 |
| 4-6 | 6 家族でのテスト運用・調整 | ☐ |

> **なぜ Phase 4-4/4-5 を今コミットしないか**：同期レイヤーは「ローカル `S.*` ⇄ クラウド」の
> 双方向マージ（last-write-wins）と Realtime 購読を伴い、**誤実装すると家族の体調・家計・写真等の
> 機微データを破壊しうる**。バックエンド（テーブル/RLS）が存在しない状態では実装の動作確認が一切
> できず、2 アカウントでの受け入れ検証も不可能。よって「検証できない機微データ同期コードを本番
> ライブへ盲目的に投入しない」方針とし、Phase 4-1（SQL 実行）完了後にテスト可能な状態で着手する。
> これは CLAUDE.md §10.2（家族情報に関わる重大変更は人間確認必須）/ §13（高プライバシーデータ保護）に沿う。

...
最終更新：2026-05-17 / 状態：**設計（オーナー確認 → セットアップ → 実装の順で進行）**
...
|---|---|---|
| 4-0 | 本設計書（確認）| ☑ 本書 |
| 4-1 | オーナーが Supabase プロジェクト作成 + SQL 実行 | ☐ 要オーナー |
| 4-2 | ログイン / サインアップ / 招待コード画面（UI）| ☐ |
| 4-3 | supabase-js 連携・認証フロー | ☐ |
| 4-4 | family_data 同期レイヤー（読み書き + 移行）| ☐ |

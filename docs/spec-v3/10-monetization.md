...
**文書番号**: SPEC-v3-10 ／ **版**: 1.1 ／ **作成日**: 2026-07-07 ／ **改訂日**: 2026-07-08（表示整合 3 点の解消を反映） ／ **正本**
...
| 広告モデル不採用 | 家族の信頼と両立しないため広告は掲載しない（`01-product-vision.md` §5.3）。無料プランにも広告はなく、「広告なし」をプレミアム訴求に使うこともしない（2026-07-08 に訴求文言を全廃・§3.1） |
...
> 注記 2（2026-07-08 解消済み）: かつて存在した `adFree`（広告なし）訴求は、広告モデル不採用方針（§1.1）と矛盾するため
> 定数・UI 文言ともに全廃した。プレミアム訴求は実在機能（Hoku 無制限・保存枠無制限・OCR 月 30 回・アバター・テーマ）のみで行う。
...
| 価格 | ¥0 | ¥480/月 |
| 予定表の読み取り | 月1回 | 月30回 |
| 写真・書類の保存枠 | 写真20枚ほか | 無制限 |
| Hoku 利用 | 1 日 5 回 | 無制限 |
| プレミアムアバター | — | ✓ |
| 写真/書類管理 | 基本機能 | 拡張機能 |
| 今後の限定機能 | — | ✓ |

フィーチャーカード（L21116）: 「予定表の読み取りが月30回まで」「写真・書類の保存枠が無制限（無料は写真20枚・書類15件・メモ20件）」「プレミアムアバターが使える（18種類以上・ファンタジーやドラゴンシリーズ）」。

> 注記（2026-07-08 解消済み）: かつての「ストレージ 500MB/20GB」訴求は実装（§3.2 の件数上限）と乖離していたため、
> 比較表・フィーチャーカード・設定メニュー・ストレージ管理モーダルのすべてを件数ベースの正直な表現へ置換した。
> 容量ベースの訴求は、実際にクラウドストレージ計測を実装した時のみ復活できる。
...
- 「ご契約と解約は、この「プレミアム」画面からいつでも行えます。」
- 「家族で利用する場合も、代表者1名の契約でプレミアム機能を利用できます。」
- 「同じアカウントでログインすると、ご契約状態は自動的に反映されます。」

> **解消済み（2026-07-08）**: 旧文言「App Store のアカウント設定から」「購入を復元」は IAP 前提で Stripe/β 実態と
> 矛盾していたため、配布チャネル非依存の文言（アプリ内画面からの解約・ログインによる権利自動反映＝サーバ権利 §5 と一致）へ
> 置換した。決済モーダルの「安全なお支払い」文もβ実態（請求なし・外部送信なし）を明示する文へ改めた。
> IAP でネイティブ配布する際は、ストア要件の文言（購入の復元等）を **その時点の実装に合わせて**再度追加すること（NFR-703）。
...
> **既知の実装ギャップ（P1・実課金の有効化前に必須）**: UI は「代表者 1 名の契約で家族全員が利用できる」（規約・FAQ）と
> 家族単位課金を訴求するが、現行の権利は**本人単位**である（`fl_my_premium` は本人行のみ返すビュー、`isPremiumUser` は
> 本人同期キーで家族共有キーではない＝2026-07-08 実測）。`STRIPE_ENABLED=true` にする前に、
> 「family_id 内に有効な entitlement 保持者がいれば家族全員 premium」とするサーバ側ビュー/RPC の追加が必要
> （DB/RLS 変更＝CLAUDE.md §10.2 の人間確認必須事項）。β の現在は実請求が発生しないため利用者への実害はない。
...
| 文言整合 | 規約文は配布チャネル非依存の文言に統一済み（§4.3・2026-07-08）。**IAP 導入時はストア要件の文言（購入の復元等）を実装と同時に追加する**（NFR-703） |
...
- 要整合 3 項目（①ストレージ容量訴求 ②IAP 規約文言 ③広告なし訴求）は **2026-07-08 にすべて解消済み**（§3.1 / §3.3 / §4.3 の各注記参照）。以後、実装と矛盾する訴求を追加しないこと。
